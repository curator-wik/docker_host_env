FROM prophusion/prophusion-apache
...
# Add CORS header to everything so the angular app can be hosted from the development nodejs server on some other port
RUN /usr/sbin/a2enmod headers
COPY cors-header.conf /etc/apache2/conf-enabled/cors-header.conf
